...
d.manga_voiceovers = d.manga_voiceovers.filter(v => {
  ...
  if (rejectRe.test(t)) { removed++; removedList.push(`[KEYWORD] "${t}" ≠ ${manga.title_ru}`); return false; }
  ...
  if (ruWordCount === 1 && ruLower.length < 15) {
    ...
    if (!inTitlePosition && !quoted && !afterOz) {
      removed++; removedList.push(`[1WORD] "${t}" ≠ ${manga.title_ru}`); return false;
    }
    return true;
  }
  ...
  if (ruWordCount === 2 && ruLower.length < 20) {
    if (ruLower.length >= 5 && vt.includes(ruLower)) {
      ...
      removed++; removedList.push(`[2WORD] "${t}" ≠ ${manga.title_ru}`); return false;
    }
  }
  ...
  if (ruWords.length <= 1) {
    removed++; removedList.push(`[FEWWORDS] "${t}" ≠ ${manga.title_ru}`); return false;
  }
